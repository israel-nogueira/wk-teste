...
|-------------------------------------------------------------------
|	Essa classe é original de github.com/raelgc/template
|	Foi modificada por mim para pequenos ajustes
|-------------------------------------------------------------------
*/
	namespace app;
	...
	class template {
		...
		public function __set($varname, $value){
			// if($this->debugVars==true && !$this->exists($varname)) throw new \RuntimeException("A variável $varname não existe!");
			if($this->debugVars==false && !$this->exists($varname) ){
				// ...
			}else{
				$stringValue = $value;
				if(is_object($value)){
		 ...
		 * @param   string  $varname    template var name
		 */
		public function debugVars($debug=true){
			$this->debugVars = $debug;
			return $this;
		}
		 ...
		 * @return    void
		 */
		public function existsBlock($block=null) {
			return in_array($block,$this->blocks);
		}
		protected function loadfile($varname, $filename) {

			if(!is_string($filename) || $filename=="" || strlen($filename)==0){
				throw new \InvalidArgumentException("O arquivo $filename não é um arquivo válido");

			}elseif(is_string($filename) && !file_exists($filename)){

				$str = preg_replace("/<!---.*?--->/smi", "", $filename);
				$this->setValue($varname, $str);
				$blocks = $this->identify($str, $varname);
				$this->createBlocks($blocks);

			}elseif(file_exists($filename) && !$this->isPHP($filename)){
		...
		public function block($block, $append = true) {
			/*
			|------------------------------------------------------------------
			|	ao invés de printar erro, eu ignoro
			|	pois no framework tem páginas que podem não conter o bloco
			|	e nao podemos parar por causa disso
			|-------------------------------------------------------------------
			/**/
			/**/	if(!in_array($block, $this->blocks)) return false;
			//------------------------------------------------------------------
		...
		*/
		...
		public function loop( Array $array, String $block ){
			foreach ( $array as $value) {
				$this->VALUE = $value['ID'];
				$this->LABEL = $value['TITULO'];
				$this->block($block);  
			}
			return $this;
		}
		 ...
		 * content.
		 */
		public function setJSON($_JSON=[],$block=false) {
			if(is_object($_JSON) || is_array($_JSON)){
				foreach ($_JSON as $key => $value) {
					// if($this->debugVars==true && !$this->exists($key)) throw new \RuntimeException("A variável $key não existe!");
					if($this->debugVars==false && !$this->exists($key) ){
						//...
					}else{
						$stringValue = $value;
						if(is_object($value)){
							$this->instances[$key] = $value;
							if(!isset($this->properties[$key])) $this->properties[$key] = array();
							if(method_exists($value, "__toString")) $stringValue = $value->__toString();
							else $stringValue = "Object";
						}
						$this->setValue($key, $stringValue);
					}
					if($block!=false){ $this->block($block);}

				}
			}else{
				throw new \InvalidArgumentException("setJSON formato incorreto:".__LINE__);
			}
			return $this;
		}
	}
